...
from sklearn.preprocessing import QuantileTransformer, MultiLabelBinarizer, LabelEncoder
...
class Dataset:
    ...
    def __init__(self, dataset, random_state, data_path='./data', normalize=False,
        ...
        if quantile_transform:
            ...
            quantile_train = quantile_train.astype(np.float64)
            if quantile_noise:
...
def fetch_RATING(path):
    ...
    X_validation = validation_data[['UserID', 'MovieID']].values
    y_validation = validation_data['Rating'].values

    return dict(
        X_train=X_train.astype(np.float32), y_train=y_train,
        X_valid=X_validation.astype(np.float32), y_valid=y_validation,
...
def fetch_CLASS(path):
    ...
    for col in ['Title' ,'Director', 'Cast', 'Certificate', 'Plot', 'Languages']:
        le = LabelEncoder()
        train_data[col] = le.fit_transform(train_data[col])
        train_label_encoders[col] = le
    ...
    for col in ['Title' ,'Director', 'Cast', 'Certificate', 'Plot', 'Languages']:
        le = LabelEncoder()
        val_data[col] = le.fit_transform(val_data[col])
        val_label_encoders[col] = le

    # 多标签独热编码
    mlb = MultiLabelBinarizer()
    train_data['Genre'] = train_data['Genre'].str.split('|')
    test_data['Genre'] = test_data['Genre'].str.split('|')
    val_data['Genre'] = val_data['Genre'].str.split('|')
    train_genre_encoded = pd.DataFrame(mlb.fit_transform(train_data['Genre']), columns=mlb.classes_).values
    test_genre_encoded = pd.DataFrame(mlb.fit_transform(test_data['Genre']), columns=mlb.classes_).values
    val_genre_encoded = pd.DataFrame(mlb.fit_transform(val_data['Genre']), columns=mlb.classes_).values

    # 合并编码后的数据
    train_data['Runtime'] = [int(i[0]) if type(i)!=float else 100 for i in train_data['Runtime'].str.split(' ') ]
    test_data['Runtime'] = [int(i[0]) if type(i)!=float else 100 for i in test_data['Runtime'].str.split(' ') ]
    val_data['Runtime'] = [int(i[0]) if type(i)!=float else 100 for i in val_data['Runtime'].str.split(' ') ]

    movid_train = train_data[['Year', 'MovielensID', 'Runtime']]
    train_data_encoded = pd.concat([train_data[['Title' ,'Director', 'Cast', 'Certificate', 'Plot', 'Languages']], movid_train], axis=1)
    movid_test= test_data[['Year', 'MovielensID', 'Runtime']]
    test_data_encoded = pd.concat([test_data[['Title' ,'Director', 'Cast', 'Certificate', 'Plot', 'Languages']], movid_test], axis=1)
    movid_val = val_data[['Year', 'MovielensID', 'Runtime']]
    val_data_encoded = pd.concat([val_data[['Title' ,'Director', 'Cast', 'Certificate', 'Plot', 'Languages']], movid_val], axis=1)

    # print(train_data_encoded.shape, train_genre_encoded.shape)
    # print(test_data_encoded.shape, test_genre_encoded.shape)
    # print(val_data_encoded.shape, val_genre_encoded.shape)

    return dict(
        X_train=train_data_encoded.astype(np.float32), y_train=train_genre_encoded.astype(np.float32),
        X_valid=val_data_encoded.astype(np.float32), y_valid=val_genre_encoded.astype(np.float32),
        X_test=test_data_encoded.astype(np.float32), y_test=test_genre_encoded.astype(np.float32)
    )
