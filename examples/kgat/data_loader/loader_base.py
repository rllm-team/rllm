...
class DataLoaderBase(object):
    ...
    def statistic_cf(self):
        self.n_users = max(
            ...
            max(self.cf_test_data[0])) + 1
        self.n_items = max(
            max(self.cf_train_data[1]),
            max(self.cf_test_data[1])) + 1
    ...
    def sample_pos_items_for_u(self, user_dict, user_id, n_sample_pos_items):
        ...
        while True:
            ...
            pos_item_idx = \
                np.random.randint(low=0, high=n_pos_items, size=1)[0]
    ...
    def sample_neg_items_for_u(self, user_dict, user_id, n_sample_neg_items):
        ...
        while True:
            ...
            neg_item_id = \
                np.random.randint(low=0, high=self.n_items, size=1)[0]

            if neg_item_id not in pos_items \
                    and neg_item_id not in sample_neg_items:
    ...
    def generate_cf_batch(self, user_dict, batch_size):
        ...
        if batch_size <= len(exist_users):
            ...
            exist_users_list = list(exist_users)
            batch_user = \
                [random.choice(exist_users_list) for _ in range(batch_size)]
    ...
    def sample_pos_triples_for_h(self, kg_dict, head, n_sample_pos_triples):
        ...
        while True:
            ...
            pos_triple_idx = \
                np.random.randint(low=0, high=n_pos_triples, size=1)[0]
            tail = pos_triples[pos_triple_idx][0]
            relation = pos_triples[pos_triple_idx][1]

            if relation not in sample_relations \
                    and tail not in sample_pos_tails:
    ...
    def sample_neg_triples_for_h(
        ...
        while True:
            ...
            if (tail, relation) not in pos_triples \
                    and tail not in sample_neg_tails:
    ...
    def generate_kg_batch(self, kg_dict, batch_size, highest_neg_idx):
        ...
        if batch_size <= len(exist_heads):
            ...
            exist_heads_list = list(exist_heads)
            batch_head = \
                [random.choice(exist_heads_list) for _ in range(batch_size)]
