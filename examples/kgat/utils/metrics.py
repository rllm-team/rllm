...
def calc_mse(input_tensor, target):
    return torch.nn.MSELoss()(input_tensor, target)
...
def calc_recall(rank, ground_truth, k):
    ...
    """
    return len(
        set(rank[:k]) & set(ground_truth)) / float(len(set(ground_truth)))
...
def average_precision(hit, cut):
    ...
    hit = np.asarray(hit)
    precisions = \
        [precision_at_k(hit, k + 1) for k in range(cut) if len(hit) >= k]
    if not precisions:
        return 0.
...
def ndcg_at_k_batch(hits, k):
    ...
    idcg = np.sum(
        (2 ** sorted_hits_k - 1) / np.log2(np.arange(2, k + 2)),
        axis=1
        )
...
def calc_metrics_at_k(
    ...
    """
    test_pos_item_binary = \
        np.zeros([len(user_ids), len(item_ids)], dtype=np.float32)
