...
class Trainer(nn.Module):
    ...
    def train_on_batch(self, *batch, device):
        ...
        # x_batch = torch.as_tensor(x_batch, device=device, dtype=torch.float32)#.double()    ############
        # y_batch = torch.as_tensor(y_batch, device=device, dtype=torch.float32)#.double()
    ...
    def evaluate_classification_error(self, X_test, y_test, device, batch_size=4096):
        ...
        with torch.no_grad():
            logits = process_in_chunks(self.model, X_test, batch_size=batch_size)
            y_pred = (logits > 0.5).int()  # 阈值为0.5，将概率转换为二进制预测
            macro_f1 = f1_score(y_test.cpu().numpy(), y_pred.cpu().numpy(), average='macro')
            error_rate = 1 - macro_f1
        return error_rate

    def evaluate_classification_error_micro(self, X_test, y_test, device, batch_size=4096):
        X_test = torch.as_tensor(X_test, device=device)
        y_test = torch.as_tensor(y_test, device=device)
        self.model.train(False)
        with torch.no_grad():
            logits = process_in_chunks(self.model, X_test, batch_size=batch_size)
            y_pred = (logits > 0.5).int()  # 阈值为0.5，将概率转换为二进制预测
            macro_f1 = f1_score(y_test.cpu().numpy(), y_pred.cpu().numpy(), average='micro')
            error_rate = 1 - macro_f1
        return error_rate
