...
train = pd.read_csv('yourpath/regression/ratings/train.csv')
validation= pd.read_csv('yourpath/regression/ratings/validation.csv')
test = pd.read_csv('yourpath/regression/ratings/test.csv')
users = pd.read_csv('yourpath/regression/users.csv')
movies = pd.read_csv('yourpath/regression/movies.csv')

##### merge data through userID and MovieID
def merge_data(data):
    data_movies = pd.merge(data, movies, left_on='MovieID', right_on='MovielensID')
...
for col in categorical_columns:
    print(f"{col}: {train_merge[col].nunique()}, {valid_merge[col].nunique()}, {test_merge[col].nunique()}")

##### define categorical features for categorical embeddings
unused_feat = ['Url','MovieID','Timestamp','Plot','Title','Genre','Director','Cast','Language']
