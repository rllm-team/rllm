...
def adj_matrix_to_list(adj_matrix):
    ...
    Args:
        adj_matrix (COO Sparse Tensor):
        The adjacency matrix representing the connections between nodes.
    """
    adj_list = {}
...
def sampling(src_nodes, sample_num, neighbor_table):
    ...
    for sid in src_nodes:
        # 从节点的邻居中进行有放回地进行采样
        res = np.random.choice(neighbor_table[sid.item()], size=(sample_num, ))
        results.append(res)
