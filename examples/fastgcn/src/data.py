...
class DataLoader:
    ...
    def __init__(self, v, vmeta, y, ymeta, e, emeta, node_index=None, edge_weight=None):
        ...
        for i in range(self.e_class):
            ei = e[i]
            ev = torch.ones(
                ei.shape[1]) if edge_weight is None else edge_weight[i]
            # print(ei)
    ...
    def __getitem__(self, key):
        ...
        edge_index = [self.e.e[_[0]]
                      for _ in self.e.meta if _[1] in key and _[2] in key]
        edge_weight = [self.e.w[_[0]]
                       for _ in self.e.meta if _[1] in key and _[2] in key]
    ...
    def __repr__(self):
        ...
        for _ in self.x.meta:
            res3 += '{}: {} * {}; '.format(_,
                                           self.v_num[_], self.x[_].shape[1])
        res3 += '\n'
        res4 = '\tlabel '
        for _ in self.y.meta:
            if len(self.y[_].shape) > 1:
                res4 += '{}: {} * {}; '.format(_,
                                               self.v_num[_], self.y[_].shape[1])
            else:
