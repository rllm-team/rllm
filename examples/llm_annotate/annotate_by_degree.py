...
parser.add_argument("--use_cache", type=bool, default=True, help="Wether to use cache")
...
real_labels = torch.tensor([label_names.index(_) if _ in label_names else 0 for _ in target_table.df[target_table.target_col].tolist()])
pseudo_labels = annotate(args.dataset, label_names, target_table, train_mask | val_mask, llm, use_cache=args.use_cache)

y = pseudo_labels.long().to(device)
y[test_mask] = real_labels.long().to(device)[test_mask]
target_table.y = y
