...
class TAPEDataset(Dataset):
    ...
    def download(self):
        ...
        path_text = download_url(
            self.urls['original text'][self.name],
            self.raw_dir,
            f'{self.name}_orig.zip'
        )
        path_llm = download_url(
            self.urls['LLM responses'][self.name],
            self.raw_dir,
            f'{self.name}.zip'
        )
        path_preds = download_url(  # noqa
            self.urls['GPT predictions'][self.name],
            self.raw_dir,
            f'{self.name}.csv'
        )
    ...
    def _get_raw_text_pubmed(self):
        ...
        # parse nodes
        with open(
            osp.join(path, 'Pubmed-Diabetes.NODE.paper.tab'), 'r'
        ) as node_file:
