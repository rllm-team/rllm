...
class GATConv(torch.nn.Module):
    ...
    def __init__(
        ...
        # - out_channels is the size of each head's output feature
    ...
    def forward(
        ...
        scores_source_selected = scores_source.index_select(
        ...
        # leakyReLU + softmax
        scores_edge = self.leakyReLU(
            scores_source_selected + scores_target_selected)
        ...
        out_nodes_features = self.skip_concat(
            inputs, nodes_features_aggregated
        )
    ...
    def skip_concat(self, in_channels, out_channels):
        if not out_channels.is_contiguous():
            out_channels = out_channels.contiguous()

        if self.concat:
            out_channels = out_channels.view(
                -1, self.heads * self.out_channels)
        else:
            out_channels = out_channels.mean(dim=self.head_dim)

        if self.skip_connection:
            if out_channels.shape[-1] == in_channels.shape[-1]:
                out_channels += in_channels.unsqueeze(1)
            else:
                out_channels += self.skip(in_channels).view(
                    -1, self.heads*self.out_channels
                )

        if self.bias is not None:
            out_channels += self.bias

        return (
            out_channels
        )

    def score_edge_wiht_neighborhood(
        self,
        scores_edge_exp,
        idx_target,
        num_nodes
