...
from rllm.utils import seg_softmax, seg_sum
...
class HGTConv(torch.nn.Module):
    ...
    def message(self, q_i, k_j, v_j, rel, tgt_index, num_nodes):
        ...
        alpha = self.dropout(seg_softmax(alpha, tgt_index, num_nodes))
        # out: (N, H, D)
    ...
    def aggregate(self, msg, tgt_index, num_nodes, aggr):
        if aggr == "sum":
            ...
            return seg_sum(data=msg, segment_ids=tgt_index, num_segments=num_nodes)
...
# Deprecated functions, use `seg_sum` and `seg_softmax` instead.
