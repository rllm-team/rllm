...
class PerFeatureEncoderLayer(Module):
    """Generic transformer encoder layer with one self-attention block.

    This layer consists of a single multi-head self-attention block and
    feedforward neural networks (FFNs/MLPs).
    ...
    Args:
        ...
        attention_between_features:
            Deprecated compatibility flag; no-op in single-MHA mode.
        ...
        self_attn_between_items_module:
            Optional custom self-attention module (items attention).
            If provided, it is used instead of the default `MultiHeadAttention`.
        ffn_module:
            Optional custom feedforward module used as the main FFN block.
            If provided, it is used instead of the default `MLP`.
    """
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        self_attn_between_items_module: nn.Module | None = None,
        ...
        if self_attn_between_items_module is not None:
            self.self_attn = self_attn_between_items_module
        else:
    ...
    def _call_module(
        ...
        **extra_kwargs: Any,
        ...
        """Call injected attention/FFN modules with graceful signature fallback."""
        ...
        if "add_input" in params or accepts_var_kwargs:
            call_kwargs["add_input"] = add_input
        if "allow_inplace" in params or accepts_var_kwargs:
            call_kwargs["allow_inplace"] = allow_inplace

        if src is not None and ("src" in params or accepts_var_kwargs):
            call_kwargs["src"] = src

        if src is not None and "key_value" in params:
            call_kwargs["key_value"] = src
        if src is not None and "kv" in params:
            call_kwargs["kv"] = src

        output: Any
        try:
            if src is not None and ("src" not in params and not accepts_var_kwargs):
                output = module(x, src, **call_kwargs)
            else:
                output = module(x, **call_kwargs)
        except TypeError:
            # Fall back to minimal signatures for generic user-defined modules.
            if src is not None:
                try:
                    output = module(x, src)
                except TypeError:
        ...
        if not isinstance(output, torch.Tensor):
            raise TypeError(
                "Injected module must return a Tensor (or tuple with Tensor first).",
            )
